...
function parseArgs(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    else if (arg === "--refresh-frames-if-running") options.refreshFramesIfRunning = true;
...
async function main() {
  ...
  if (options.refresh || options.refreshIfRunning || options.refreshFramesIfRunning) {
    ...
    if (refreshed.length === 0) {
      if (options.refreshIfRunning || options.refreshFramesIfRunning) {
